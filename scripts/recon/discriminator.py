...
class BaselineDiscriminator(nn.Module):
    ...
    def generator_loss_fn(self, generated_images: torch.Tensor, target_generator=None) -> torch.Tensor:
        ...
        loss = torch.log(torch.sum(torch.sigmoid(fake_logits.squeeze()), targets))
        return loss
    ...
    def discriminator_loss_fn(self, generated_images: torch.Tensor, real_images: torch.Tensor, 
        ...
        # Вычисляем MSE лоссы
        real_loss = torch.log(torch.sum(torch.sigmoid(real_logits.squeeze()), real_targets))
        fake_loss = torch.log(torch.sum(torch.sigmoid(fake_logits.squeeze()), fake_targets))
